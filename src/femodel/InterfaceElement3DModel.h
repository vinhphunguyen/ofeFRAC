...
#ifndef INTERFACE_ELEMENT_3D_MODEL_H
#define INTERFACE_ELEMENT_3D_MODEL_H
...
//=======================================================================
//   class InterfaceElement3DModel
//=======================================================================


class InterfaceElement3DModel : public Model
{
 public:

  typedef InterfaceElement3DModel  Self;
  typedef Model                     Super;
 ...
 protected:
  ...
  virtual                  ~InterfaceElement3DModel       ();
 ...
 private:
  ...
  // compute the transformation matrix generally
  // at a Gauss point
  ...
  // compute the transformation matrix for linear elements
  // tetrahedron and hexahedron elements, using the opposite
  // vertex to determine outward or inward normal.
  // coord is the coord of the surface.
  ...
  inline void                addConstraint_ 
    ...
    ( int xdof,  int ydof, int zdof,
      int xdofM, int ydofM, int zdofM );
 ...
 private:
  ...
  int                       nodeCount2_; // half of nodeCount_
  idx_t                     dofCount_;   // no.of.dofs per element
  idx_t                     dofCount2_;   // no.of.dofs per element / 2
  idx_t                     ipCount_;    // num of GP per element

  Ref<FShape>               shape_;
  Ref<SShape>               sshape_; // used to compute the grads of shape funcs
  ...
  std::vector<int>          oppositeVertices_; 
  
  bool                      initConsDone_;
  bool                      largeDisp_;

  ShapeFunc                 getShapeFuncs_;
  UpdateCoordFunc           getUpdateCoordFuncs_;
};
...
inline void InterfaceElement3DModel::addConstraint_
   ...
   int xdofM, int ydofM, int zdofM )
{
  ...
  cons_->addConstraint ( zdof, zdofM, 1.0 );
}
